    ...
    if value[:status] == :in_progress
      ...
      value[:attacks].each do |attack|
        if attack[:damage] == 0
          ...
          if value[:dead_raiders].include? attack[:defender]
      ...
      unless value[:xp].nil?
        value[:xp].each do |xp|
          msg += "#{xp[:name]} receives #{DoubloonScape::RAID_WIN_LVL_AMOUNT}% XP!\n"
        end
      end
      ...
      unless value[:gold].nil?
        value[:gold].each do |gold|
          msg += "#{gold[:name]} receives #{gold[:amount]} gold!\n"
        end
      end
    elsif value[:status] == :won
      msg += "The port has been deserted and the lootbox bank unguarded!\n"
